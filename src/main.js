...
const createCard = (color) => {
  return `
  <article class="card card--${color} card--${color === `black` ? `edit` : `repeat`}">
...
const renderCards = (cardTemplate, number) => {
  ...
  for (let i = 0; i < number; i++) {
    boardTasksElement.insertAdjacentHTML(`afterbegin`, cardTemplate(cardColors[generateNumber(0, 3)]));
  }
};

renderCards(createCard, 7);

const filterLabelElements = document.querySelectorAll(`.filter__label`);

for (let filterLabel of filterLabelElements) {
  filterLabel.addEventListener(`click`, () => {
    renderCards(createCard, generateNumber(3, 10));
  });
}
